...
from .delegation import StudentExpertDelegationMechanism
...
class StudentExpertEnsemble:
    def __init__(
        ...
        # make sure delegation mechanism is a StudentExpertDelegationMechanism
        if not isinstance(delegation_mechanism, StudentExpertDelegationMechanism):
            raise ValueError(
                "StudentExpertEnsemble must be initialized with a StudentExpertDelegationMechanism"
            )
    ...
    def predict_proba(self, X, y, record_pointwise_accs=False):
        ...
        """
        expert_gurus_and_weights = (
            self.delegation_mechanism.get_expert_gurus_with_weights(self.voters)
        )

        # get the predicted probas from each guru
        all_probas = []
        for guru, weight in expert_gurus_and_weights.items():
            probas = guru.predict_proba(X)
            # if a guru has weight 2, append their predictions twice, etc.
            for i in range(weight):
                all_probas.append(probas)
    ...
    def learn_batch(self, X, y):
        ...
        """
        for voter in self.voters:
            if not self.delegation_mechanism.voter_is_student(voter):
                # Do not train voters while they're delegating
                continue
