...
np.random.seed(0)
active_voters = batch_metric_values["simple_delegating_ensemble"][0]["active_voters-train"]
train_accs = batch_metric_values["simple_delegating_ensemble"][0]["batch_train_acc"]

plot_accuracy_and_active_voters(train_accs, active_voters, title="Ensemble Training Accuracy")


for i in exp.test_splits:
    plt.axvline(x=i, color="red", linestyle="--")
np.random.seed(0)
active_voters = batch_metric_values["simple_delegating_ensemble"][0]["active_voters-test"]
test_accs = batch_metric_values["simple_delegating_ensemble"][0]["batch_test_acc"]
plot_accuracy_and_active_voters(test_accs, active_voters, title="Ensemble Testing Accuracy")
